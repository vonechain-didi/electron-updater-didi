...
class AppUpdater extends events_1.EventEmitter {
    ...
    async executeDownload(taskOptions) {
        ...
        downloadOptions.onAbort = ()=>this.emit(main_1.ABORT_DOWNLOAD);
        ...
        if (cachedUpdateFile != null) {
            this.emit(main_1.DOWNLOAD_PROGRESS, {
                percent:100
            });
    ...
    abortDownload(){
        this.once(main_1.ABORT_DOWNLOAD,()=>{
            if(this.cachePath){
                this.removeCachePackage(this.cachePath);
            }
        })
        this.httpExecutor && this.httpExecutor.abortRequest();
    }
    async removeCachePackage(path){
        const {opendir, unlink } = fs.promises;
        try {
            const files = await opendir(path);
            for await (const f of files){
                // new Notification({ title: '移除文件', body: f.name }).show()
                await unlink(`${path}/${f.name}`)
            }

            this.emit(main_1.ABORT_DOWNLOAD);
            this.emit(main_1.DOWNLOAD_PROGRESS, {
                percent:0
            });
            ...
            this._logger.info(`Failed to delete file folder, please check the path ${path}`)
        }
